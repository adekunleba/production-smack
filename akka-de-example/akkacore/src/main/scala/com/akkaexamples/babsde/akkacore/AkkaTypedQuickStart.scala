...
import com.akkaexamples.babsde.akkacore.GreaterMain.SayHello
...
//        message.from ! Greeter.Greet(s"$n", context.self)
        bot(n, max)
      }
    }
  }
...
object AkkaTypedQuickStart extends App {
  val greaterMain: ActorSystem[GreaterMain.SayHello] = ActorSystem(GreaterMain(), "AkkaQuickStart")
  greaterMain ! SayHello("Charles")
  greaterMain ! SayHello("Maxwell")
  greaterMain ! SayHello("JohnPowell")
}
