...
import com.akkaexamples.babsde.akkastream.Route.GetWebsocketFlow
...
object WebSocketStreamServerClient extends App {
  implicit val as = ActorSystem("example")
  implicit val am = Materializer.createMaterializer(as)

  Http()
    .newServerAt("0.0.0.0", 8123)
    .bind(Route.websocketRoute)
    .onComplete {
...
object Route {
  ...
  val websocketRoute =
    pathEndOrSingleSlash {
      ...
      val handler: ActorRef = as.actorOf(Props[ClientHandlerActor])
